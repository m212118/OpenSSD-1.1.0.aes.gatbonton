...
for k=1:length(classifier1)
    ...
    for i = 1:size(comb,1)
        ...
        % reduce features to the two selected classes
        mask = false(n,1);
        for j = 1:size(comb,2)
            mask = mask | strcmp(labels{1}, comb(i,j));
        end
        X = feat_matrix(mask,:); Y = labels{1}(mask);
        filelabels=labels{2}(mask);
        
        % report performance of binary classifier on subset of classes
        command=sprintf('performance(i) = CrossValTestByFile(X,Y,filelabels,@model_%s);', classifier1(k));
        try
        eval(command);
...
for k=1:length(classifier2)
    ...
    for K=1:2:7
        ...
        for i = 1:size(comb,1)
            ...
            % report performance of binary classifier on subset of classes
            try
                command=sprintf('performance(i) = CrossValTestByFile(X,Y,filelabels,@model_%s);', classifier2(k));
                eval(command);
                fprintf('%s - %s correct classification rate:\t%g\n', ...
                    comb{i,1}, comb{i,2}, performance(i).CorrectRate);
            catch
