...
static BOOL32 install_block_zero(void)
{
	...
	// TODO STEP 5 - AES Key storage

	// Key
	// TODO generate key	
	buf =  {0x2b7e1516, 0x28aed2a6 0xabf71588, 0x09cf4f3c};
	// TODO If password is set encrypt data

	for (rbank = 0; rbank < NUM_BANKS_MAX; rbank++)
	{
		// Looking for an empty bank
		if ((BANK_BMP & (1 << rbank)) == 0)
			continue;
		page_offset =  FW_PAGE_OFFSET + firmware_image_pages;

		if (write_to_flash(buf, rbank, page_offset, 32) == FAIL)
		{
			return FAIL;
		}
