...
void ftl_read(UINT32 const lba, UINT32 const total_sectors)
{
	...
	while (sectors_remain != 0)	// one page per iteration
	{
		...
		else
		{
            ...
            // Icahn old version, for example, if the host request to read unwritten sector 0 after programming in sector 1, Jasmine would send 0x00...00 to host.
