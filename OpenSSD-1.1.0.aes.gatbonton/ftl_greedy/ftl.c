...
// 0 is not a mode
#define AES_ECB_KEY_1 10
#define AES_ECB_KEY_2 20
#define AES_ECB_KEY_3 30
#define NORMAL_MODE 40
int mode = NORMAL_MODE;

#include "aes.h"

    // Note, this key is given as a test value in aes.c
    uint8_t key_1[] = {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};
    uint8_t key_2[] = {0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55, 0x55};
    uint8_t key_3[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
...
static void   write_page(UINT32 const lpn, UINT32 const sect_offset, UINT32 const num_sectors, int printBytes);
...
void ftl_open(void)
{
    ...
    uart_printf("Encrypt FTL, Mode: = %d", mode);

    if(mode == AES_ECB_KEY_1)
    {
        // TBD: Handling both ECB and CBC
        AES_init_ctx(&ctx, key_1);
    }
    else if(mode == AES_ECB_KEY_2)
    {
        // TBD: Handling both ECB and CBC
        AES_init_ctx(&ctx, key_2);
    }
    else if(mode == AES_ECB_KEY_3)
    {
        // TBD: Handling both ECB and CBC
        AES_init_ctx(&ctx, key_3);
    }
...
void ftl_read(UINT32 const lba, UINT32 const num_sectors)
{
    ...
    while (remain_sects != 0)
    {
        ...
        // read_dram_32(PAGE_MAP_ADDR + lpn * sizeof(UINT32))
        vpn  = get_vpn(lpn);
        CHECK_VPAGE(vpn);
...
// These globals need to be intialized outside of ftl_write
int numWrites = 0;
int printingPoss = 0;
...
void ftl_write(UINT32 const lba, UINT32 const num_sectors)
{
    ...
    if(printingPoss)
    {
        uart_printf("%d: ftl_write: numWrites= %d,", mode, numWrites);
        uart_printf("num_sectors = %d", num_sectors);
    }
    int printBytes = 0;

    // // Turn on based on UART
    // UINT8 turnOn = uart_readAvail();

    printingPoss = 1;
    // Enable encryption
    // PHILIP: Always encrypt with constant key
    mode = AES_ECB_KEY_1;
    AES_init_ctx(&ctx, key_1);
    uart_printf("Going to AES key1 mode");

    if(printingPoss && num_sectors ==256)
    {
        printBytes = 1;
        led(1);
    }
    ...
    while (remain_sects != 0)
    {
        ...
        write_page(lpn, sect_offset, num_sectors_to_write, printBytes);
...
static void write_page(UINT32 const lpn, UINT32 const sect_offset, UINT32 const num_sectors, int printBytes)
{
    // What does this function ASSERT do?
    // It checks the value of an expression that we expect to be true under normal circumstances.
    // If expression is zero, the assert macro writes a message to stderr and terminates the program by calling abort.
    ...
    //necessary variables for encryption
    UINT32 old_page_offset,old_column_cnt;
    ...
    // Save old page offset and column_cnt
    // They'll be overwritten by the encryption
    old_page_offset = page_offset;
    old_column_cnt = column_cnt;
    ...
    // Do encryption
    if(mode != NORMAL_MODE && printBytes)
    {
        #define numWords  128/4
        //page offset and column cnt are counts of sectors
        for(int i = 0; i<old_column_cnt*BYTES_PER_SECTOR;i+=128)//+= 4 b/c GETREG returns UINT32
        {
            UINT32 fakeAddr = WR_BUF_PTR(g_ftl_write_buf_id)+(old_page_offset*BYTES_PER_SECTOR)+i;

            UINT32 bigBuf[2*numWords+64];
	    //forced alignment

            UINT32* buf = bigBuf-((int)bigBuf%132)+132;

            UINT32 real_addr = DRAM_BASE + (fakeAddr - DRAM_BASE)/128*132 + (fakeAddr - DRAM_BASE)%128;	//uart_printf("buf: %x, realAddr: %x",buf, real_addr);

            // Break up buffer into words
            for(int j = 0; j<numWords;j++)
            {
	        buf[j] = GETREG(real_addr + j*4);//+0x01020304;
            }

            // Encrypt buffer using ECB
            // Note ECB requires blocks to be 16 bytes long
            // Length of buffer is 128 bytes.
            int length = 128;

            // Cast buffer from 32 bit uint to 8 bit uint
            uint8_t* buf_ptr = ((uint8_t*) buf);

            // Loop over 16 byte blocks
            for (int j = 0; j < length; j += AES_BLOCKLEN)
            {
                AES_ECB_encrypt(&ctx, buf_ptr);
                buf_ptr += AES_BLOCKLEN;
            }

            // Uncomment this to do CBC instead
	    // AES_CBC_encrypt_buffer(&ctx, ((uint8_t*) buf), 128);

            // Copy encrypted data to fakeAddr
	    mem_copy(fakeAddr,buf,numWords*4);
        }
    }
