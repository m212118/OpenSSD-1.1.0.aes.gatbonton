...
// Servicing a SIRQ, checking for command
// If load a file, triggers interrupt on SATA
// When a read request is triggered, the read ATA command is sent form the
// event queue, then FTL sends FCP command to flash memory controller to read the
// data from NAND flash. Finally, the data is transferred to the assigned SATA read
// buffer
void Main(void)
{
	while (1)
	{
		if (eventq_get_count())
		{
			...
			{
				// the LBA field contains the LBA of the first logical sector to be transferred.
				// Each transport standard defines how the LBA field is mapped to the appropriate fields or register
