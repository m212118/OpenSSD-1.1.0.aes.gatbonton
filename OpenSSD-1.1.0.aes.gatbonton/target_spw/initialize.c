...
// Probably bootloader code
/*
		Jasmine board first initializes the hardware component. Hardware component then calls
		the ftl open function to initialize the FTL. Initialization process of Greedy FTL is described
		below:
		1. Set the bad block bitmap table by reading scan list from the VBK #0 by calling
		build bad blk list function

		2. Execute format operation until format mark is written in VBLK #0
				*The format function is responsible for initializing the metadata of
		SRAM/DRAM and deleting the scope except in VBLK #0
	 			*Write initialized metadata in meta scope of NAND flash and format mark in
		VBLK #0.
				*If it is not booting for the first time i.e., no need to execute format, then load
		the FTL metadata from NAND flash
		3. Initialize the FTL buffer pointer

		*If the value of the GPIO #0 equals 0 (Jumper placed at Normal mode)
			**ROM code identifies the address and numbers of the installed NAND flash, reads the
				firmware image from block #0 and finally loads it to SRAM. Then, address remap
				operation called as ‘jump’ moves the CPU address space 0 from ROM to SRAM. Thus,
				initiates the firmware. Firmware loading halts and the board goes to the Factory mode,
				if the contents of block #0 is missed or corrupted or if the page #4 signature
				(0xC0C2E003) of the block #0 is lost
*/
...
void init_jasmine(void)
{
	...
	// TODO
	// Before anything, request a password from user through UART
	// User password to generate KEK and decrypt DEK
	// Check decrypted DEK against 128 bit block of encrypted values
	// If correct, continue read and write?

	// if password is set
		// Turn on based on UART
		UINT8 pass = uart_readAvail();
		while(pass != 'y') {
			uart_printf("Incorrect password %c", pass);
			// 1. Request password
			pass = uart_readAvail();
			// 2. generate KEK
			// 3. read from VBLOCK #0 AES offset to read encrypted DEK
			// 4. Decrypt DEK
			// 5. Read from VBLOCK #0 AES Table offset to read encrypted data?
			// 6. if decrypted data matches continue
			// 7. else request password again

		}
	// else retreive unecrypted key from VBLOCK #0 AES
